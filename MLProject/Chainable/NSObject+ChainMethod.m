       ...
       const char *type = [methodSignature getArgumentTypeAtIndex:i];
        if (*type == '{') {
            hasStructParam = YES;
                ...
                [boxValueString appendFormat:@"mlChain__MASBoxValue(metamacro_at(%ld, __VA_ARGS__))", i - 2];
        ...
        for (NSInteger i = 2; i < numberOfArguments; i++) {
            ...
            for (MLChainStructModel *model in structModels) {
                if ([model.encodeTypeString isEqualToString:encodeTypeString]) {
                    structboxValueString = [NSString stringWithFormat:@"mlChain__MASBoxValue(%@(__VA_ARGS__))", model.makePrefixString];
                }
            }
